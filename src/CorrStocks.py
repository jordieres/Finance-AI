...
This module calculates corr between Stocks for the training period.
'''
import os
import warnings
import pandas as pd
...
def main(args) -> None:
    ...
    for scenario in config['scenarios']:
        ...
        win = list_win_size[0]
        ish = ish + 1
        tr_tst = scenario['tr_tst'][0]
        scen_name = scenario['name']
        fdat1 = out_path + "/input/{}-corr.pkl".format(scen_name)
        stocklst[scen_name] = {}
        for ticker in stock_list:
            ...
            stocklst[scen_name][ticker] = stock
        #
        # Claculate corrs.
        cmat = pd.DataFrame(data=None,index=stock_list,columns=stock_list)
        for tickeri in stock_list:
            idi = list(stocklst[scen_name][tickeri].serial_dict[\
                       'INPUT_DATA'].keys())[0]
            vy  = stocklst[scen_name][tickeri].serial_dict[\
                       'INPUT_DATA'][idi]["trainY"]
            for tickerj in stock_list:
                idj = list(stocklst[scen_name][tickerj].serial_dict[\
                           'INPUT_DATA'].keys())[0]
                vz  = stocklst[scen_name][tickerj].serial_dict[\
                       'INPUT_DATA'][idj]["trainY"]
                cvyz= pd.concat([vy,vz], axis=1).dropna()
                cmat[tickeri][tickerj] = cvyz[0].corr(cvyz[1])
        scenlst[scen_name]=cmat
    #
        with open(fdat1, 'wb') as file:
            pickle.dump(out_path, file)
            pickle.dump(fdat1, file)
            pickle.dump(scenlst, file)
