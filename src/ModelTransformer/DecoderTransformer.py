...
def positional_encoding(seq_len: int, dim_model: int) -> tensor:
    pos = torch.arange(seq_len, dtype=float32).reshape(1, -1, 1)
    dim = torch.arange(dim_model, dtype=float32).reshape(1, 1, -1)
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query: tensor, key: tensor, value: tensor, mask: bool = False) -> tensor:
        ...
        step_1 = torch.cat([h(query, key, value, self.mask) for h in self.attention_heads], dim=-1)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # d_q: dimension of the query

        super().__init__()

        dim_q = dim_k = int(torch.div(d_model, num_heads, rounding_mode='trunc'))
    ...
    def forward(self, src: tensor, memory: tensor) -> tensor:
        ...
        step_1 = self.attention_1(src, src, src)

        # Step 2: Apply the attention layer
        step_2 = self.attention_2(step_1, memory, memory)

        # Step 3: Apply the feed forward layer
        step_3 = self.feed_forward(step_2)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList(
            [TransformerDecoderLayer(d_model=d_model, num_heads=num_heads, units_hidden_layer=units_hidden_layer,
    ...
    def forward(self, src: tensor, memory: tensor) -> tensor:
        ...
        src += positional_encoding(seq_len=src.size(1), dim_model=self.d_model)

        # Step 2: Apply the decoder layers
        for layer in self.layers:
            src = layer(src=src, memory=memory)

        # Step 3: Apply the final linear layer
        src = nn.Linear(self.d_model, src.size(2))(src)
