...
from utils_vv_tfg import load_preprocessed_data, save_data, denormalize_data, select_features
...
def transformer_fun(transformer_parameters: dict, train_X: pd.DataFrame, 
                    train_y: pd.DataFrame, test_X: pd.DataFrame, test_y: pd.DataFrame, 
                    Y: pd.DataFrame, vdd: dict, epochs: int, bsize: int, 
                    nhn: int, win: int, n_ftrs: int, ahead: int, stock: str, 
                    test_X_idx: pd.Index, k: int, seed: int) -> dict:
    """
    ...
    nit = 0
    lloss = float('nan')
    model = TransformerL(input_dim=k, embed_dim=nhn, 
...
def main(args):
    ...
    for scenario in config['scenarios']:
        ...
        for i, config_transformer in enumerate(transformer_configs):
            ...
            scenario_features = config_transformer['features']
        ...
        for win_size in list_win_size:
            for tr_tst in list_tr_tst:
                ...
                fdat = '{}/output/{}/{}/{}-{}-m-output.pkl'.format(processed_path,win,tr_tst,scenario['name'],tmod)
