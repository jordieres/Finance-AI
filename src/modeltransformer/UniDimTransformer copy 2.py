...
def transformer_fun(transformer_parameters, train_X, train_y, test_X, test_y,
    ...
    nit = 0
    lloss = float('nan')
    model = Transformer(input_dim=n_ftrs, embed_dim=nhn, 
    ...
    while torch.isnan(torch.Tensor([lloss])) and nit < 5:
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(np_train_X), bsize):
                ...
                output = model(batch_np_train_X)  # Pass the same input as both encoder and decoder
            ...
            with torch.no_grad():
                y_hat = model(np_test_X)  # Pass the same input as both encoder and decoder
...
def main():
    ...
    for scenario in config['scenarios']:
        ...
        for tr_tst in list_tr_tst:
            ...
            fdat = f'/home/vvallejo/Finance-AI/dataprocessed/output/{win}/{tr_tst}/test-{tmod}-output.pkl'
